...
void motor_control_init(void)
{
    ...
    // 获取电机数据指针
    motor_control.left_motor = get_left_motor_measure_point();  // 获取左电机数据
    motor_control.right_motor = get_right_motor_measure_point(); // 获取右电机数据
...
void motor_control_calc(void)
{
    ...
    motor_control.left_current = (int16_t)pid_calc(&motor_pid[0], 
                                                motor_control.left_motor->int16_vel, 
                                                motor_control.left_speed_set);
    
    // 使用PID计算右电机电流
    motor_control.right_current = (int16_t)pid_calc(&motor_pid[1], 
                                                 motor_control.right_motor->int16_vel, 
                                                 motor_control.right_speed_set);
    
    // 发送控制命令到两个电机
    CAN_cmd(motor_control.left_current, motor_control.right_current);
}